#!/usr/bin/env python3
"""
ACO for multi-UAV AoI-aware IoT collection (range-aware) - FINAL

Features:
 - Ants construct feasible IoT visit lists per UAV (visit reachable IoTs).
 - Unvisited IoTs are assigned to nearest UAV as fallback.
 - Fitness = weighted normalized sum of Peak AoI, Avg AoI, Total Distance, minus coverage.
 - Infeasible solutions (UAV exceeding Tmax) receive large penalty.
 - Pheromone on directed IoT edges + start->node tau. All ants deposit; elitist deposit included.
 - Prints per-iteration best fitness, peak, avg, total_distance, coverage.
 - No 2-opt/local search; no greedy seeding by default.
"""
import random
import copy
import math
...
ELITIST_BOOST = 6.0  # multiplier for best-so-far reinforcement (0 to disable)
NO_IMPROVE_LIMIT = 40
TAU0 = 1.0      # initial pheromone
SEED_WITH_GREEDY = False  # we won't use greedy seeding by default
GB_IMMEDIATE_INSERT_THRESHOLD = 2 * R_GBS  # when to insert GB immediately
...
W_AVG = 0.3
W_DIST = 0.15
W_COV = 0.25  # coverage reduces fitness (higher coverage -> lower fitness)
EPS = 1e-9
...
# ----------------------------
# Evaluation functions (AoI + travel)
# ----------------------------
...
# ----------------------------
# Small greedy builder (not used for seeding, but useful for debugging)
# ----------------------------
...
def construct_solution(pheromone, start_tau, iot_list, gb_list, uav_list,R_I=R_IOT, R_G=R_GBS):
    ...
    for u in uav_list:
        ...
        # ensure end present and clean
        solution_uavs[u.id].append(('end', None, u.end))
        solution_uavs[u.id] = clean_path_remove_consecutive_gbs(solution_uavs[u.id], u)

    # fallback: assign unvisited IoTs
    if all_unvisited:
        for iid in list(all_unvisited):
            iobj = iot_map[iid]
            best_u = min(uav_list, key=lambda uu: dist(solution_uavs[uu.id][-2][2], iobj.pos))
...
# ----------------------------
# Fitness: peak, avg, distance, coverage (normalized)
# ----------------------------
def compute_fitness_for_solution(solution_uavs, iot_objs, gb_objs, base_uavs,
                                 Tmax_norm=T_MAX, area_size=AREA_SIZE,
                                 w_peak=W_PEAK, w_avg=W_AVG, w_dist=W_DIST, w_cov=W_COV):
    # Build copies
    ...
    # conservative max dist bound: each UAV crosses diagonal once
    ...
    # scalar fitness (lower is better); coverage reduces the score
    fitness = (w_peak * peak_norm) + (w_avg * avg_norm) + (w_dist * dist_norm) - (w_cov * coverage_norm) + infeasible_penalty
    fitness = (w_peak * peak_norm) + (w_avg * avg_norm) +  infeasible_penalty

    # return fitness and diagnostics
    return fitness, peak_val, avg_val, coverage_norm, total_dist, per_uav_finish_times, feas, uav_copies

# ----------------------------
# ACO main loop (all ants deposit + elitist)
# ----------------------------
def run_aco(iot_objs, gb_objs, uav_list):
    ...
    for it in range(ACO_ITERS):
        ...
        for a in range(NUM_ANTS):
            ...
            fitness, peak, avg, coverage, total_dist, per_uav_finish_times, feas, uav_copies = compute_fitness_for_solution(sol, iot_objs, gb_objs, uav_list)
            ants_solutions.append((sol, fitness, peak, avg, coverage, total_dist, per_uav_finish_times, feas, uav_copies))
            ants_fitnesses.append(fitness)
            if fitness < best_fitness:
                best_fitness = fitness
                best_solution = sol
                best_info = (fitness, peak, avg, coverage, total_dist, per_uav_finish_times, feas, uav_copies)

        # update no_improve correctly
        ...
        ## elitist deposit: reinforce best-so-far
        ...
        # numerical bounds (prevent blow-up)
        np.clip(pheromone, 1e-9, 1e9, out=pheromone)
        np.clip(start_tau, 1e-9, 1e9, out=start_tau)

        # printing iteration info: include total distance + coverage
        ...
        if no_improve >= NO_IMPROVE_LIMIT:
            print(f"No improvement for {NO_IMPROVE_LIMIT} iterations -> early stop at iter {it}")
            break
...
if __name__ == '__main__':
    ...
    if best_info is not None:
        bf = best_info
        print(f"Best info (fitness, peak, avg, coverage, total_dist): {bf[0]:.6f}, {bf[1]}, {bf[2]}, {bf[3]:.3f}, {bf[4]:.2f}")
    ...
    if best_sol is not None:
        ...
        for u in uav_copies:
            ...
            print("")
        # plot
        plot_solution(iot_objs, gb_objs, uav_copies, title="ACO final (range-aware)")
    else:
