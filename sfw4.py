#!/usr/bin/env python3
"""
sfw_multi_uav_aoi_weighted_enhanced.py

SFW-based solver for AoI multi-UAV data collection with two enhancements:
  1) Sigmoid-based Time-Smooth Transition (TSG)
  2) Adaptive Fitness-Guided Mutation Strategy (AFGMS) with hybrid Cauchy-Gaussian noise
...
Run: python sfw_multi_uav_aoi_weighted_enhanced.py
...
# ----------------------------
# Enhancement parameters (TSG + AFGMS)
# ----------------------------
# --- TSG (Sigmoid time-smooth transition) ---
K_SIG = 10.0      # steepness of sigmoid (larger -> sharper transition)
T0 = 0.5          # center of transition (fraction of max iters)
...
# ----------------------------
# SFW main loop with enhancements (TSG + AFGMS)
# ----------------------------
def run_sfw(iot_objs, gb_objs, uav_list, pop=POP, max_iters=MAX_ITERS):
    ...
    for it in range(1, max_iters + 1):
        # --- TSG: time-smoothed transition using sigmoid ---
        # compute normalized time in [0,1]
        t_norm = it / float(max_iters)
        # sigmoid blending factor (0->exploration, 1->exploitation)
        tau = 1.0 / (1.0 + math.exp(-K_SIG * (t_norm - T0)))
        # tau is used to blend exploration and exploitation steps smoothly.
        #
        # parameters used in the original loop
        ...
        for i in range(pop):
            # Instead of hard if T<c1:... else:..., we smoothly blend:
            # exploration_step: random local addition (diversity)
            expl = X[i] + (LB + (UB - LB) * np.random.rand(dim))
            # exploitation step: guided by best_position (if available)
            if best_position is None:
                exploit = X[i]
            else:
                s = r1 * 20.0 + r2 * 20.0
                if s > 20.0:
                    # levy-guided heavy exploitation step (if levy chosen)
                    y = random.randrange(pop)
                    exploit = best_position + X[i] * L[y] * k
                else:
                    XG = best_position * C_PARAM
                    exploit = XG + (best_position - X[i]) * p
            # Blend with tau: more tau -> more exploitation
            Xnew[i] = (1.0 - tau) * expl + tau * exploit

            # Bound
            Xnew[i] = np.maximum(Xnew[i], lb)
            Xnew[i] = np.minimum(Xnew[i], ub)

        # evaluate Xnew and replace if better (original replacement logic)
        for i in range(pop):
