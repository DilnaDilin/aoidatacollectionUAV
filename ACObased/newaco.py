...
def construct_solution(pheromone, start_tau, iot_list, gb_list, uav_list, R_I=R_IOT, R_G=R_GBS):
    """
    Constructs a solution for all UAVs using ACO.
    Opportunistic GBS upload if UAV is within 2*R_G of a GBS.
    """
    n = len(iot_list)
    ...
    for u in uav_list:
        cur_pos = np.array(u.start, dtype=float)
        cur_time = 0
        prev_idx = None
        while True:
            # build list of candidate IoTs that can be visited
            candidates = []
            for iid in list(all_unvisited):
                ...
                # Determine nearest GBS for upload
                ...
                # Only consider candidates that can finish upload within Tmax
                if t_upload <= u.Tmax:
                    candidates.append((iid, i_wp, nearest_g.id, g_wp, t_upload))

            if not candidates:
                break  # no more reachable IoTs
            ...
            # Add IoT waypoint to path
            solution_uavs[u.id].append(('iot', iid, i_wp))
            all_unvisited.discard(iid)

            # Opportunistic GBS upload if UAV is within 2*R_G
            nearby_gbs = [g for g in gb_list if dist(i_wp, g.pos) <= 2 * R_G]
            if nearby_gbs:
                nearest_g = min(nearby_gbs, key=lambda g: dist(i_wp, g.pos))
                g_wp = point_on_line_at_distance_from_target(i_wp, nearest_g.pos, R_G)
                solution_uavs[u.id].append(('gb', nearest_g.id, g_wp))
                cur_pos = np.array(g_wp, dtype=float)
                cur_time = int(cur_time + travel_time(i_wp, g_wp, u.V))
            else:
                cur_pos = np.array(i_wp, dtype=float)
                cur_time = int(t_upload)

            prev_idx = iid - 1

        # Ensure UAV ends at 'end' waypoint
        solution_uavs[u.id].append(('end', None, u.end))
        solution_uavs[u.id] = clean_path_remove_consecutive_gbs(solution_uavs[u.id], u)

    # Fallback: assign remaining IoTs to nearest UAV
    if all_unvisited:
        for iid in list(all_unvisited):
            iobj = iot_map[iid]
            ...
            nearest_g = min(gb_list, key=lambda g: dist(i_wp, g.pos))
            g_wp = point_on_line_at_distance_from_target(i_wp, nearest_g.pos, R_G)
            solution_uavs[best_u.id] = solution_uavs[best_u.id][:-1] + [
                ('iot', iid, i_wp),
                ('gb', nearest_g.id, g_wp),
                ('end', None, best_u.end)
            ]
