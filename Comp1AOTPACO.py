...
def build_greedy_solution(iot_objs, gb_objs, uav_list, R_I=R_IOT, R_G=R_GBS):
    ...
    if unvisited:
        for iid in list(unvisited):
            ...
            unvisited.discard(iid)
    # ensure consecutive gb cleaned and start/end present
    for u in uav_list:
...
if __name__ == '__main__':
    ...
    if best_info is not None:
        bf = best_info
        print(
            f"Best info (fitness, peak, avg, coverage, total_dist): {bf[0]:.6f}, {bf[1]}, {bf[2]}, {bf[3]:.3f}, {bf[4]:.1f}")

    if best_sol is not None:
        # Recompute detailed evaluation
        ...
        V_D_glob, V_U_glob, per_uav_lengths, per_uav_finish_times, feas_flags = evaluate_all_uavs(uav_copies, iot_objs,
                                                                                                  gb_objs)
        aoi_dict, peak, avg, coverage = compute_aoi_from_VU(V_U_glob, iot_objs)
        ...
        # Plot solution
        plot_solution(iot_objs, gb_objs, uav_copies, title="ACO multi-UAV (range-aware) result")
    else:
